...
class ComplexSearch {
  ...
  private function get_tags_checkbox( $name, $value, $checked, $text, $vertical, $is_label_front='', $class='', $id='' ) {
    ...
    $id    = $id!='' ? ' id="' . $id . '"' : '';
    $tag  = '';
    $tag .= '<li class="check-btn-1">';
    $tag .= '<input id="' . 'id-' . $value . '"';
    $tag .= 'type="checkbox" ';
    $tag .= 'name="' . $name . '[]" ';
    $tag .= 'value="' . $value . '"';
    $tag .= $checked;
    $tag .= $class . $id;
    $tag .= '>';
    $tag .= '<label for="' . 'id-' . $value . '">' . $text . '</label>';
    $tag .= '</li>';
    return $tag;
  }
        ...
        foreach ($gloup_value_arr as $input_type => $input_values) {
          foreach ($input_values as $input_value) {
            if(isset($_GET[$input_value['name']]) && !empty($_GET[$input_value['name']])) {
              ...
              foreach ($key_arr as $tkey) {
                if( !empty($input_value['_' . $tkey]) ) {
                  foreach ($input_value['_' . $tkey] as $tkey2 => $tvalue2) {
                    if( $get_tax_ids == $tkey2 ) {
                      $input_value[$tkey] = $tvalue2;
                    }
                  }
  ...
  */
   ...
   * リセットボタン
   */
  public function form_input_reset( $label='', $redirect_url='', $form_id = '' ) {
    $redirect_url = home_url( $redirect_url );
    echo '<input id="osSubqueryReset" type="button" value="' . $label . '" onClick="location.href=\'' . $redirect_url . '\';">';
